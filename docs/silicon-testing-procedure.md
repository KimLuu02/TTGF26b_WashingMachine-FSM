# Silicon Testing Procedure: Washing Machine Controller
...
| Cycle | Applied Inputs<br>`(reset_in, start, door_closed, mode_select)` | Expected State | Expected Digital Outputs<br>`(water_valve, wash_motor, spin_motor, done_led)` | 
| :---: | :--- | :---: | :--- | 
...
### Test Case 3: Water Fill Cycle and Timer Selection ($timer\_sel = 0$)
...
### Test Case 4: Extended Wash Cycle and Timer Selection ($timer\_sel = 1$)
Validate that the dynamic timing logic correctly identifies a high-duration state operation and scales the cycle runtime proportionally to 5 clock cycles when active.
...
### Test Case 5: Forced Runtime Interrupt via Master Reset
Verify asynchronous-like override behavior. Activating the asynchronous master clear system at any point during active operations must instantly drop power to all actuator controls and override pending states.

| Cycle | Applied Inputs<br>`(reset_in, start, door_closed, mode_select)` | Expected State | Expected Digital Outputs<br>`(water_valve, wash_motor, spin_motor, done_led)` |
| :---: | :--- | :---: | :--- | 
| **1** | `1, 1, 1, 0` *(Normal Run)* | **WASH** | `0, 1, 0, 0`  |
| **2** | `0, 1, 1, 0` *(Forced Interruption)*| **IDLE** | `0, 0, 0, 0`  |
| **3** | `1, 0, 0, 0` *(Safe Recovery)* | **IDLE** | `0, 0, 0, 0` |